...
from and lessons learned during the development of ABM, and even uses the same stream 
reader, so ABM must be installed to be able to access the bouquet creation option of 
this plugin.
...
we have to rely on users submitting the data and due to this not all areas work 100% 
...
a list of duplicated channels, or worse still, scanning in the weak transponder and the 
...
using the tools built in to enigma and it stay where you put it during subsequent scans, 
but may be move by other processes that also write to the bouquet index such as ABM. The 
