...
#
printf "Creating temporary file autoBLchanger-py.pot\n"
find $findoptions .. -name "*.py" -exec xgettext --no-wrap -L Python --from-code=UTF-8 -kpgettext:1c,2 --add-comments="TRANSLATORS:" -d autoBLchanger -s -o autoBLchanger-py.pot {} \+
$localgsed --in-place autoBLchanger-py.pot --expression=s/CHARSET/UTF-8/
printf "Creating temporary file autoBLchanger-xml.pot\n"
which python
if [ $? -eq 0 ]; then
	find $findoptions .. -name "*.xml" -exec python xml2po.py {} \+ > autoBLchanger-xml.pot
else
	find $findoptions .. -name "*.xml" -exec python3 xml2po.py {} \+ > autoBLchanger-xml.pot
fi
printf "Merging pot files to create: enigma2.pot\n"
cat autoBLchanger-py.pot autoBLchanger-xml.pot | msguniq -s --no-wrap --no-location -o autoBLchanger.pot -
#printf "remove pot Creation date\n"
#$localgsed -i -e'/POT-Creation/d' autoBLchanger.pot
printf "remove temp pot files\n"
rm autoBLchanger-py.pot autoBLchanger-xml.pot
