...
class PiconsUpdaterView(ConfigListScreen, Screen):
    ...
    def downloadBackgroundImages(self):
        ...
        """
        if os.path.isdir(TMP_BG_PATH) is False:
            os.makedirs(TMP_BG_PATH, 493)
        if os.path.isdir(TMP_FG_PATH) is False:
            os.makedirs(TMP_FG_PATH, 493)
        for data in getBackgroundList():
            localPiconBgPath = TMP_BG_PATH + '/' + os.path.basename(data['bg'])
            if os.path.isfile(localPiconBgPath) is False:
                DownloadJob(data['bg'], localPiconBgPath, self.__bgDownloadFinished, self.__bgDownloadFailed)
            else:
                self.__bgDownloadFinished()
            if 'fg' in data:
                localPiconFgPath = TMP_FG_PATH + '/' + os.path.basename(data['fg'])
                if os.path.isfile(localPiconFgPath) is False:
    ...
    def getBackgroundImagePath(self):
        backgroundUrl = self.getCurrentBackground()['bg']
        localPiconBgPath = TMP_BG_PATH + '/' + os.path.basename(backgroundUrl)
        return localPiconBgPath

    def getForegroundImagePath(self):
        if 'fg' in self.getCurrentBackground():
            foregroundUrl = self.getCurrentBackground()['fg']
            localPiconFgPath = TMP_FG_PATH + '/' + os.path.basename(foregroundUrl)
    ...
    def startDownloadPicons(self):
        try:
            if os.path.isdir(getPiconsPath().getValue()) == False:
                os.makedirs(getPiconsPath().getValue(), 493)
        except:
            pass
    ...
    def __downloadAllFinished(self, downloadPicons):
        ...
        for channel in downloadPicons.channelsNotFoundList:
            channelsNotFoundFile.write(os.path.basename(channel[1]) + ';' + channel[0] + ';' + channel[1] + '\n')
    ...
    def __mergePicons(self, *args):
        ...
        factor = self.getCurrentBackground()['factor']
