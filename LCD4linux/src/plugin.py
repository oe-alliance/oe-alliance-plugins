...
from dateutil.parser import parse as parseD
...
from time import strftime, localtime, mktime, time, sleep, timezone, altzone, daylight
...
from enigma import eEPGCache, eServiceReference, eServiceCenter, iPlayableService, getDesktop, getEnigmaVersionString, eEnv, ePicLoad
...
LCD4picon = "%senigma2/picon/" % LCD4share  # /usr/share/enigma2/picon/
LCD4fonts = resolveFilename(SCOPE_FONTS)  # /usr/share/fonts/
LCD4config = "%slcd4config" % LCD4enigma2config  # /etc/enigma2/lcd4config
LCD4plugin = "%sExtensions/LCD4linux/" % LCD4enigma2plugin  # /usr/lib/enigma2/python/Plugins/Extensions/LCD4linux/
Data = "%sdata/" % LCD4plugin  # /usr/lib/enigma2/python/Plugins/Extensions/LCD4linux/data/
LCD4default = "%sdefault.lcd" % Data
WetterPath = "%swetter/" % LCD4plugin
MeteoPath = "%smeteo/" % LCD4plugin
FONTdefault = "%snmsbd.ttf" % LCD4fonts
FONT = FONTdefault
ClockBack = "%sPAclock2.png" % Data
Clock = "%sClock" % Data
RecPic = "%srec.png" % Data
TMP = "%s/" % realpath("%stmp" % LCD4plugin) if islink("%stmp" % LCD4plugin) == True else "/tmp/"
TMPL = "%slcd4linux/" % TMP
xmlPIC = "%sl4ldisplay.png" % TMP
xmlPICtmp = "%sl4ldisplaytmp.png" % TMP
Fritz = "%sfritz.txt" % TMPL
FritzFrame = "%sfritzcallframe.png" % Data
FritzRing = "%sfritzcallring.png" % Data
FritzPic = "%sfritzpic.png" % Data
PIC = "%sdpf" % TMPL
PICtmp = "%sdpftmp" % TMPL
PIC2 = "%sdpf2" % TMPL
...
msn_list = ("4", "3", "3", "4", "4", "3", "37", "38", "39", "3", "9", "9", "9", "9", "9", "9", "9", "9", "9", "11", "12", "12", "12", "12", "10", "40", "11", "12", "11", "14", "16", "43", "18", "7", "6", "5",
	"42", "46", "41", "20", "22", "21", "19", "20", "19", "19", "19", "23", "0", "2", "30", "28", "30", "26", "0", "1", "2", "25", "36", "24", "17", "31", "29", "27")
openweather_list = ("200", "201", "202", "210", "211", "212", "221", "230", "231", "232", "300", "301", "302", "310", "311", "312", "313", "314", "321", "500", "501", "502", "503", "504", "511",
	"520", "521", "522", "531", "600", "601", "602", "611", "612", "615", "616", "620", "621", "622", "701", "711", "721", "731", "741", "751", "761", "762", "771", "781", "800", "801", "802", "803",
	"804", "900", "901", "902", "903", "904", "905", "906", "n800", "n803", "n802")
...
LCD4linux.WetterApi = ConfigSelection(choices=[("MSN", _("MSN")), ("OPENWEATHER", _("OpenWeatherMap")), ("WEATHERUNLOCKED", _("WeatherUnlocked"))], default="MSN")
	...
	if isdir(P2C):
		if not isfile(PD):
			...
			if not PY3:
				...
				name = normalize('NFKD', text_type(str("" + channelname), 'utf-8', errors='ignore')).encode('ASCII', 'ignore')
			else:
			...
			name = sub(r'[^a-z0-9]', '', name.replace('&', 'and').replace('+', 'plus').replace('*', 'star').lower()) + ".png"
			if not PY3:
...
def getHTMLwwwConvertapi(fn, www, pw, ph):
	...
	callInThread(threadDownloadPage, url, filename, boundFunction(HTMLwwwConvertapiDownloadFinished, filename), HTMLwwwConvertapiDownloadFailed)
...
def getHTMLwww(fn, url):
	filename = WWWpic % str(fn)
	L4log("downloading HTMLwww from", url)
	callInThread(threadDownloadPage, url, filename, boundFunction(HTMLwwwDownloadFinished, filename), HTMLwwwDownloadFailed)
...
def InitWebIF():
	...
	if i > 0 and len(glob(LCD4enigma2plugin + "Extensions/WebInterface/__init__.py*")) > 0:
		...
		root.putChild(b"data", static.File(ensure_binary(Data[:-1])))
		if exists(LCD4enigma2plugin + "Extensions/LCD4linux/WebInterface/web/external.xml"):
			try:
			...
			addExternalChild(("lcd4linux", root))
			L4log("use old WebIf")
		if exists(LCD4enigma2plugin + "Extensions/OpenWebif/pluginshook.src"):
			try:
						...
						for M in range(1, int(mailserver.stat()[0]) + 1):
							...
							for R in mailserver.retr(M)[1]:
								if R.upper().startswith("FROM:"):
									...
									Date = R[R.find(" "):].strip()
									Date = "- %s" % Date.split("+")[0].split(",")[-1].strip()
								if From != "" and Subj != "":
									break
...
############# End Helper functions
		...
		for wwwURL in elements:
			try:
				if len(wwwURL[0]) > 4:
					...
					callInThread(threadGetPage, boundFunction(feedurl, headers=Header, timeout=10), boundFunction(self.downloadwwwBoxCallback, i), boundFunction(self.downloadwwwBoxError, i))
		...
		for wwwURL in elements:
			...
			try:
				if len(wwwURL[0]) > 4:
					...
					callInThread(threadGetPage, boundFunction(feedurl, headers=Header, timeout=10), (self.downloadwwwBoxTimerCallback, i), (self.downloadwwwBoxTimerError, i))
	...
	def downloadWetter(self, ort, wetter):
		if self.NetworkConnectionAvailable or self.NetworkConnectionAvailable is None:
			...
			if LCD4linux.WetterApi.value == "MSN":
				...
				city = quote(ort).split(",")[0]
				self.feedurl = "http://www.msn.com/%sin-%s?weadegreetype=C" % (localisation.get(la, "en-us/weather/forecast/"), city)  # fallback to general localized url
				L4log("URL:", self.feedurl)
				callInThread(threadGetPage, self.feedurl, boundFunction(self.downloadMSNListCallback, wetter), self.downloadListError)
				...
				L4log("URL1:", self.feedurl)
				callInThread(threadGetPage, self.feedurl, boundFunction(self.downloadOpenListCallback, wetter), self.downloadListError)
				...
				lang = ""
				if "." in ort:
					lang = "&lang=%s" % ort.split(".")[0]
				if len(LCD4linux.WetterApiKeyWeatherUnlocked.value.split()) == 2:
					apkey = "?app_id=%s&app_key=%s" % (LCD4linux.WetterApiKeyWeatherUnlocked.value.split()[0], LCD4linux.WetterApiKeyWeatherUnlocked.value.split()[1])
				city = "%s" % quote(ort)
				self.feedurl = "http://api.weatherunlocked.com/api/current/%s%s%s" % (city, apkey, lang)
				L4log("URL1:", self.feedurl)
				callInThread(threadGetPage, self.feedurl, boundFunction(self.downloadUnlockedListCallback, wetter), self.downloadListError)
	...
	def downloadMSNListCallback(self, ConfigWWW, page=""):
		...
		try:
			...
			page = page[startpos + len(start):endpos]
			if len(page):
				wwwWetter[ConfigWWW] = "MSN Weather is available"
				jsonData = None
				try:
					jsonData = loads(page)["WeatherData"]["_@STATE@_"]
				except Exception as jsonerr:
					L4log("Wetter%s Error: found invalid json-string. %s: %s" % (ConfigWWW, jsonerr))
					return
				if jsonData:
					self.WDay[ConfigWWW] = {}
					currdate = parseD(jsonData["lastUpdated"])
					self.WDay[ConfigWWW]["Wtime"] = currdate.strftime("%H:%M")
					self.WDay[ConfigWWW]["Wtime"] = currdate
					self.WDay[ConfigWWW]["Locname"] = jsonData["currentLocation"]["displayName"].split(",")[0]
					self.Long[ConfigWWW] = jsonData["currentLocation"]["longitude"]
					self.Lat[ConfigWWW] = jsonData["currentLocation"]["latitude"]
					self.downloadSunrise()
					L4log("lon: %s, lat: %s" % (self.Long[ConfigWWW], self.Lat[ConfigWWW]))
					current = jsonData["currentCondition"]  # collect current weather data
					self.WDay[ConfigWWW]["Temp_c"] = current["currentTemperature"]
					...
					for idx in range(6):
						High = str(forecast[idx]["highTemp"])
					...
					wwwWetter[ConfigWWW] = ""
					self.WetterOK = False
					L4log("Wetter%sdownload Error" % ConfigWWW, "%s" % self.MSNsrc)
					L4logE("Wetter Error Page:", page)
		except Exception as err:
			L4log("Wetterdownload unavailable Current Data. Error:", "%s" % err)
	...
	def downloadOpenListCallback(self, ConfigWWW, page=""):
		...
		elif r.get("name", None) is not None:
			self.WetterOK = True
			self.WDay[ConfigWWW] = {}
	...
	def downloadUnlockedListCallback(self, ConfigWWW, page=""):
		global wwwWetter
		r = {}
		try:
			page = ensure_str(page)
		...
		L4log("Wetter Ready:", "%s" % r)
		if r.get("Days", None) is not None:
			self.WetterOK = True
			L4log("Wetter%sdownloadForcast OK" % ConfigWWW)
			wwwWetter[ConfigWWW] = r
			L4log("parse Wetter %s" % ConfigWWW)
			self.WWeek[ConfigWWW] = []
			for curr in r.get("Days", []):
				L4logE(curr)
				High = Code_utf8("%d" % round(curr.get("temp_max_c", 0)))
				Low = Code_utf8("%d" % round(curr.get("temp_min_c", 0)))
				d = curr.get("date")
				dd = d.split("/")
				Day = Code_utf8(WeekDays[weekday(int(dd[2]), int(dd[1]), int(dd[0]))])
				Icon = curr["Timeframes"][4]["wx_icon"]
		...
		elif r.get("lat", None) is not None:
			self.WetterOK = True
			L4log("Wetter%sdownloadCurrent OK" % ConfigWWW)
			self.WDay[ConfigWWW] = {}
			self.WDay[ConfigWWW]["Locname"] = ""
			self.WDay[ConfigWWW]["Temp_c"] = Code_utf8(str(int(r.get("temp_c", 0))))
			self.WDay[ConfigWWW]["Hum"] = "%s%%" % r.get("humid_pct", 0)
			...
			self.WDay[ConfigWWW]["Icon"] = r.get("wx_icon", "")
	...
	def downloadSunrise(self):
		...
		if len(LCD4linux.WetterApiKeyOpenWeatherMap.value) > 599:
			...
			callInThread(threadGetPage, self.feedurl, self.downloadSunriseCallback, self.downloadSunriseError)
		else:
			...
			callInThread(threadGetPage, self.feedurl, self.downloadSunriseCallback2, self.downloadSunriseError)
		...
		if artist != "":
			...
			if LCD4linux.MPCoverType.value == "0":
				...
				try:
					...
					callInThread(threadGetPage, url, self.appleImageCallback, self.coverDownloadFailed)
				except Exception as e:
					self.LgetGoogleCover = None
				...
				if len(LCD4linux.MPCoverApiGoogle.value) < 10:
					...
					try:
						...
						L4log("Cover Search", url)
						callInThread(threadGetPage, url, self.googleImageCallback, self.coverDownloadFailed)
					except Exception as e:
						self.LgetGoogleCover = None
	...
	def appleImageCallback(self, result):
		...
		if rc != 0:
			...
			if url == "":
				...
				callInThread(threadDownloadPage, url, filename, boundFunction(self.coverDownloadFinished, filename), self.coverDownloadFailed)
		else:
			self.LgetGoogleCover = None
			self.CoverError = "E"
			self.CoverCount += 1
	...
	def getSonosPic(self, fn, url):
		...
		callInThread(threadDownloadPage, url, filename, boundFunction(self.SonosDownloadFinished, self.SonosDownloadFailed))
...
def getFont(num):
	...
	if ff[int(num)].endswith(".ttf") and isfile(ff[int(num)]):
		return ff[int(num)]
	else:
		return FONT
		...
		if self.SonosRunning and self.SonosTrack.get("album_art", "") != "":
			...
			if cover == "" and LCD4linux.MPCoverPiconFirst.value == True:
				...
				if isMediaPlayer == "record":
					...
					PIC.append(join(P2, picon))
					if not PY3:
						name = normalize('NFKD', text_type(str("" + self.Lchannel_name), 'utf-8', errors='ignore')).encode('ASCII', 'ignore')
					else:
...
# MSN Wetter
	def putWetter(workaround, draw, im):
		...
		if PICwetter[ConfigWWW] is None or ConfigType != WetterType or ConfigZoom != WetterZoom:
			...
			if len(LCD4linux.WetterPath.value) > 2 and (len(glob(join(LCD4linux.WetterPath.value, "10.*"))) > 0 or len(glob(join(LCD4linux.WetterPath.value, "01d.*"))) > 0):
				UseWetterPath = LCD4linux.WetterPath.value
			UseExt = "png"
			if len(glob(join(UseWetterPath, "*.gif"))) > 5:
				UseExt = "gif"
			L4log("Wetter Picon used: %s.%s" % (UseWetterPath, UseExt))
			...
			if ConfigType != "3" and ConfigType[0] != "4":
				...
				for curr in self.WWeek[ConfigWWW]:
					if (i < 4 and ConfigType in ["1", "2", "5"]) or (i < 5 and ConfigType in ["11", "21", "51"]) or (i < 2 and ConfigType in ["12", "22"]):
						...
						IconID = str(curr.get("IconID", "0"))
						if not isfile(join(UseWetterPath, Icon)) and IconID != "0":
							logw = str(Day) + "\t"
							logw = logw + "org: " + str(Icon) + "\t"
							...
							try:
								if "n." in str(Icon) and "n" + IconID in openweather_list:  # i==0 and
									...
									new_icon = msn_list[openweather_list.index(IconID)]
									logw = logw + "tag,new: "
								else:
									new_icon = "3200"
							except Exception as e:
								pass
							if new_icon:
								new_icon1 = new_icon + ".png"
								if isfile(join(UseWetterPath, new_icon1)):
									Icon = new_icon1
								else:
									new_icon1 = new_icon + ".gif"
									if isfile(join(UseWetterPath, new_icon1)):
										Icon = new_icon1
								L4logE("weather picon replace:\t", logw + str(Icon))
						if "." in Regen:
						...
						L4logE("Icon:", Icon)
						if isOffTime(L4LMoon, L4LSun, L4LMoon, L4LSun):
							if isfile(join(UseWetterPath, Icon.replace("." + UseExt, "_night." + UseExt))) and Icon.find(UseExt) > 1:
								Icon = Icon.replace("." + UseExt, "_night." + UseExt)
						if isfile(join(UseWetterPath, Icon)) and Icon.find(UseExt) > 1:
						...
						if ConfigType[0] == "5":
							...
							PY = int(h / 10) if len(High) > 3 or len(Low) > 3 else 0
			...
			if len(self.WDay[ConfigWWW]) != 0:
				...
				if not isfile(join(UseWetterPath, Icon)) and IconID != "0":
					new_icon = None
					logw = "now\torg: " + str(Icon) + "\t" + "ID: " + str(IconID) + "\t"
					try:
						if "n." in str(Icon) and "n" + IconID in openweather_list:
							new_icon = msn_list[openweather_list.index("n" + IconID)]
							logw = logw + "nacht,new: "
						elif IconID in openweather_list:
							new_icon = msn_list[openweather_list.index(IconID)]
					...
					if new_icon:
						...
						if isfile(join(UseWetterPath, new_icon1)):
							...
							if isfile(join(UseWetterPath, new_icon1)):
								Icon = new_icon1
						L4log("weather picon replace:\t", logw + str(Icon))
				...
				if ConfigType[0] == "4":
					if ConfigType == "4":
						Temp_c += "C"
					TextSize = int((20 if LCD4linux.WetterTrendArrows.value else 25) * Wmulti)
					font = ImageFont.truetype(ConfigFont, TextSize, encoding='unic')
					...
					if ConfigType == "3":
						...
						POSXs, POSYs = POSX, POSY + int(45 * Wmulti) + int(WetterZoom * (2.4 + 0.85 * corr))
					else:
					...
					if int(sub(r'[^0-9]+', '', Hum)) < LCD4linux.WetterRainColor2use.value:
					...
					if ConfigType[0] == "5":
						...
						ShadowText(Wim, PX, int((55 if LCD4linux.WetterTrendArrows.value else 70) * Wmulti), Cond, font, ConfigColor, ConfigShadow)
						...
						fontF = ImageFont.truetype(ConfigFont, int((int(LCD4linux.WetterExtraZoom.value) / (13.0 if LCD4linux.WetterTrendArrows.value else 10.0)) * Wmulti), encoding='unic')
...
# Clock
	def putClock(workaround, draw, im):
		...
		if ConfigType[0] == "4":
			...
			now = ""
			font = ImageFont.truetype(ConfigFont, ConfigSize, encoding='unic')
			ww, hh = getFsize(strftime("%H:%M"), font)
			font = ImageFont.truetype(ConfigFont, int(ConfigSize / 2), encoding='unic')
			ww2, hh = getFsize(strftime(_("%d.%m.%Y")), font)
			if ww2 > ww:
				ww = ww2
...
#			if self.Lpath and ":0:" not in self.Lpath and "//" not in self.Lpath:
			...
			if isData == True and ConfigBorder != "off":
				...
				if ConfigShadowBar == "true":
					if isfile(join(Data, "progress.png")):
						try:
							imW = Image.open(join(Data, "progress.png"))
					...
					if isfile(join(Data, "gradient.png")):
						...
						try:
							imW = Image.open(join(Data, "gradient.png"))
		...
		if harddiskmanager.HDDCount() > 0:
			...
			try:
				if ConfigType == "0" or (ConfigType == "1" and hddsleep == False):
					if hddsleep == True:
						imW = Image.open(join(Data, "HDDs.png"))
					else:
						imW = Image.open(join(Data, "HDDa.png"))
					xx, yy = imW.size
	...
	def putMute(workaround, draw, im):
		...
		try:
			if self.LvolM == True:
				...
				if ConfigSplit == True:
					MAX_W = int(MAX_W / 2)
				imW = Image.open(join(Data, "audio/audio_off.png"))
				xx, yy = imW.size
				x = int(float(ConfigSize) / yy * xx)
				imW = imW.resize((x, ConfigSize))
				POSX = getSplit(ConfigSplit, ConfigAlign, MAX_W, x)
	...
	def putFritz(workaround, draw, im):
		...
		if FritzTime > 1:
			...
			if int(LCD4linux.FritzLines.value) > 0:
				...
				imW = None
				x1 = 0
				if isfile(join(Data, "fritztelin.png")):
					try:
						imW = Image.open(join(Data, "fritztelin.png"))
						xx, yy = imW.size
						...
						imW = None
				imW2 = None
				if isfile(join(Data, "fritztelout.png")):
					try:
						imW2 = Image.open(join(Data, "fritztelout.png"))
						xx, yy = imW2.size
						x1 = int(float(CS) / yy * xx)
		...
		for CP in range(0, ConfigProfil):
			if len(PopMail[CP]) > 0 or LCD4linux.MailShow0.value == True:
				...
				if ConfigType[:1] == "A" or (ConfigType[:1] == "B" and PopCheck != PopMailUid[CP][0]):
					...
					POSXi = POSX
					if isfile(join(Data, "email.png")):
						try:
							imW = Image.open(join(Data, "email.png"))
							xx, yy = imW.size
							x = int(float(CS) / yy * xx)
							imW = imW.resize((x, CS))
							if LCD4linux.WetterTransparenz.value == "true":
		...
		if "B" in find:
			if isfile(join(Data, "fritztelin.png")):
				...
				if ConfigType == "0":
					...
					POSY += ConfigSize
		if "C" in find:
			if isfile(join(Data, "calendar.png")):
				try:
					imW = Image.open(join(Data, "calendar.png"))
					imW = imW.resize((ConfigSize, ConfigSize))
					if LCD4linux.WetterTransparenz.value == "true":
						imW = imW.convert("RGBA")
						self.im[im].paste(imW, (POSX, POSY), imW)
...
# show Sonnenaufgang
	def putSun(workaround, draw, im):
		...
		if isfile(join(Data, "sun.png")):
			try:
				imW = Image.open(join(Data, "sun.png"))
				imW = imW.resize((ConfigSize, ConfigSize))
				if LCD4linux.WetterTransparenz.value == "true":
					imW = imW.convert("RGBA")
					self.im[im].paste(imW, (POSX, POSY), imW)
		...
		if ConfigType == "2":
			...
			POSXi = POSX + ConfigSize + int(ConfigSize / 6)
		if isfile(join(Data, "moon.png")):
			try:
				imW = Image.open(join(Data, "moon.png"))
				imW = imW.resize((ConfigSize, ConfigSize))
				if LCD4linux.WetterTransparenz.value == "true":
					imW = imW.convert("RGBA")
					self.im[im].paste(imW, (POSX, POSY), imW)
...
# Netstmo CO2
	def putNetatmoIllu(workaround, draw, im):
		...
		if ConfigType[0] == "0":
			...
			if isfile(join(Data, "pointmask.png")):
				try:
					imM = Image.open(join(Data, "pointmask.png"))
...
# Kalender-Liste
	def putCalendarList(workaround, draw, im):
		...
		CS = int(ConfigSize * 1.8)
		w1, h1 = getFsize("A", font)
		if isfile(join(Data, "calendar.png")) and "-" not in ConfigType:
			try:
				imW = Image.open(join(Data, "calendar.png"))
				xx, yy = imW.size
				x1 = int(float(CS) / yy * xx)
