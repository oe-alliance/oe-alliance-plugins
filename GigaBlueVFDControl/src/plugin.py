...
def initLED():
	...
	if config.plugins.VFD_Giga.showClockDeepStandby.value == 'True':
		...
		forcmd = '0'
	if BOX in ("gbquad", "gb800ueplus", "gbquadplus", "gbipbox", "gbx1", "gbx2", "gbx3"):
...
class LED_GigaSetup(ConfigListScreen, Screen):
	...
	def createSetup(self):
		...
		if config.plugins.VFD_Giga.setLed.value:
			...
			if BOX not in ("gbquad", "gb800ueplus", "gb800seplus", "gbquadplus", "gbipbox", "gbultra", "gbultraue", "gbultrase", "gbx1", "gbx2", "gbx3"):
...
def main(menuid, **kwargs):
	if getImageDistro() == "openvix":
		if BOX in ('gb800se', 'gb800solo', 'gbx1', 'gbx2', 'gbx3') and menuid == "leddisplay":
