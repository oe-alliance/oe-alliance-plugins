...
class Blindscan(ConfigListScreen, Screen):
	...
	def prepareScanData(self, orb, pol, band, is_scan):
		...
		elif config.misc.boxtype.value.startswith('mara'):
			cmd = "mara_blindscan %d %d %d %d %d %d %d" % (self.feid, temp_start_int_freq, temp_end_int_freq, self.blindscan_start_symbol.value, self.blindscan_stop_symbol.value, tab_pol[pol], tab_hilow[band]) # mara_blindscan tuner_idx min_frequency max_frequency min_symbolrate max_symbolrate polarization(Vertical & Horizontal) hilow_band
