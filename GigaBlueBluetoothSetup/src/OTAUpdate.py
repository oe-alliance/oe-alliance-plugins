...
class GbRcuOtaUpdate(Screen, HelpableScreen, BluetoothTask):
	...
	def __init__(self, session, mac, gbbt_insttance, battery, app_version, new_app_version):
	...
	def appendOTAEventCallback(self, value = True):
		if value:
			...
			if self.otaEventCallback in self.gbbt.pluginOtaEventHandler:
	...
	def otaEventCallback(self, event, value):
		...
		if event == OTA_PROGRESS_DATA: # OTA_PROGRESS_DATA
			...
			text = "Wait 5 seconds while GB-BLE-RCU is rebooting..."
	...
	def disconnectDevices(self):
		...
		if pairedDevices:
			for (k, v) in pairedDevices.items():
				...
				if (name != bt_types.BT_GB_RCU_NAME) and connected:
